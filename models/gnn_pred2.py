...
class Model(M):
    def __init__(self, *args, **kwargs):
        ...
        self.fc_x_pred = nn.Linear(8 + self.config['env']['num_robots'], self.config['env']['num_robots'], bias=self.bias)
        self.fc_embedding_pred = nn.Linear(self.dim_edge, self.config['env']['num_robots'], bias=self.bias)
        self.fc_l_pred = nn.Linear(self.config['env']['num_robots'], self.config['env']['num_robots'], bias=self.bias)

        self.fc_pred_2 = nn.Linear(self.config['env']['num_robots'], self.base_hidden_size, bias=self.bias)
    ...
    def forward(self, state, action):
        ...
        visitation = state['visitation'] # (n_batch, n_nodes, n_robots)
        ...
        #pred = (1 / self.config['env']['num_robots']) * torch.ones(n_batch, n_nodes, self.config['env']['num_robots']).to(self.device)
        pred = visitation
        ...
        h1_presence = torch.relu(self.fc1_presence(h0_presence))
        ...
        for t in range(self.T1):
            ...
            embedding_dist_pred = torch.tanh( self.fc_embedding_pred(edge_dist)) # (n_batch, n_nodes, n_cities, self.base_hidden_size)
            embedding_dist_pred = pred[:, :-1, :].unsqueeze(1) * embedding_dist_pred # (n_batch, 1 -> n_nodes, n_cities, self.base_hidden_size)

            l_pred = torch.matmul(presence_in, embedding_dist_pred) # (n_batch, n_nodes, 1, self.base_hidden_size)
            l_pred = l_pred.squeeze(-2) # (n_batch, n_nodes, self.base_hidden_size)
            pred = torch.softmax(self.fc_l_pred(l_pred) + self.fc_x_pred(x), dim = -1)
