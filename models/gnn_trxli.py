...
class Model(nn.Module):
    def __init__(self, config, device='cpu', extra_gpus=None):
        ...
        self.fc_embedding_1 = nn.Linear(self.dim_edge + 2 * self.base_hidden_size, self.base_hidden_size, bias=self.bias)
        ...
        self.fc_embedding_2 = nn.Linear(self.dim_edge + 2 * self.base_hidden_size, self.base_hidden_size, bias=self.bias)
    ...
    def forward(self, state, action):
        ...
        h0_presence = torch.cat(
            [
                edge,
                h_trxli[:, :-1, :].unsqueeze(-2).repeat(1, 1, n_nodes, 1), #x_a[:, :-1, :].unsqueeze(-2).repeat(1, 1, n_nodes, 1),
                h_trxli.unsqueeze(-3).repeat(1, n_cities, 1, 1)
            ],
            dim=-1
        ) # (n_batch, n_cities, n_nodes, d)
        ...
        presence_out = torch.softmax(logit_presence / self.base_hidden_size ** 0.5, dim=-1)  # (n_batch, n_cities, n_nodes)
        ...
        embedding_dist_1 = torch.tanh(
            self.fc_embedding_1(h0_presence)).transpose(1, 2)  # (n_batch, n_nodes, n_cities, self.base_hidden_size)
        for t in range(self.T1):
            ## Original concating method
            embedding_dist_1_ = u[:, :-1, :].unsqueeze(
                1) + embedding_dist_1  # (n_batch, 1 -> n_nodes, n_cities, self.base_hidden_size)

            l_1 = torch.matmul(presence_in, embedding_dist_1_)  # (n_batch, n_nodes, 1, self.base_hidden_size)
        ...
        embedding_dist_2 = torch.tanh(
            self.fc_embedding_2(h0_presence)).transpose(1, 2)  # (n_batch, n_nodes, n_cities, self.base_hidden_size)
        for t in range(self.T2):
            embedding_dist_2_ = gamma[:, :-1, :].unsqueeze(
                1) + embedding_dist_2  # (n_batch, n_nodes, n_cities, self.base_hidden_size)

            l_2 = torch.matmul(presence_in, embedding_dist_2_)  # (n_batch, n_nodes, 1, self.base_hidden_size)
